...
/*   Updated: 2025/09/12 18:05:11 by aabelkis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* setting_key(path, equals, new_node)
   - Purpose: Extracts the key part from a "KEY=VALUE" string and assigns it to the node.
   - Parameters:
...
/* setting_value(equals, new_node)
   - Purpose: Extracts the value part from a "KEY=VALUE" string and assigns it to the node.
   - Parameters:
       * equals: pointer to '=' character in path; NULL if no value.
       * new_node: pointer to the t_env node being initialized.
   - Returns: 0 on success, 1 if allocation fails or node/key is NULL.
   - Notes: Uses ft_strdup to allocate value; sets value = NULL if no '=' present. Frees key and node on failure.
*/
...
/* setting_vars(path, equals, new_node)
   - Purpose: Initializes a new t_env node by zeroing its memory and setting the exported flag.
   - Parameters:
   ...
   - Returns: void.
   - Notes: Sets *equals to point to '=' if present; sets exported = 1. Must be called after malloc.
*/
...
/* add_nodes(env, path)
   - Purpose: Creates a new t_env node from a "KEY=VALUE" string and appends it to the linked list.
   - Parameters:
       * env: pointer to head of the t_env list; may be NULL if list is empty.
       * path: environment string to parse (e.g., "KEY=VALUE").
   - Returns: pointer to the new node, or NULL if allocation fails.
   - Notes: Calls setting_vars, setting_key, and setting_value. Adds new node to end of list and sets exported = 1.
*/
...
/* found_match(key, temp, key_len, path)
   ...
   - Notes: Frees key after successful match. Updates exported = 1. Uses ft_strdup for value allocation.
*/
...
/* update_var(path, env)
   - Purpose: Updates an existing variable if key exists; otherwise adds a new node.
   - Parameters:
       * path: environment string ("KEY=VALUE" or "KEY").
       * env: pointer to head of the t_env list.
   - Returns: 0 on success, 1 on allocation failure.
   - Notes: Calls find_key to get key, found_match to update existing nodes, and add_nodes to create new nodes.
*/
...
/* key_info(envp, j, key_one, key_two)
   - Purpose: Extracts the keys from two adjacent envp strings and returns the minimum key length.
   - Parameters:
       * envp: array of environment strings.
...
/* keys_differ(key_one, key_two, envp, j)
   - Purpose: Handles the case when two keys differ and the first should come after the second.
   - Parameters:
...
/* alphabatize(envp, j)
   - Purpose: Compares two adjacent env strings and swaps them if they are out of order.
   - Parameters:
       * envp: array of environment strings.
       * j: index of the first string to compare.
   - Returns: 1 if a swap occurred, 0 if already in order.
   - Notes: Uses keys_differ() to handle keys of different lengths and free memory.
*/
...
/* ft_bubble_sort(envp)
   - Purpose: Sorts an array of environment strings alphabetically using bubble sort.
   - Parameters:
       * envp: array of environment strings.
   - Returns: void.
   - Notes: Optimized to stop early if the array is already sorted (swap flag). Uses alphabatize for comparisons.
*/
